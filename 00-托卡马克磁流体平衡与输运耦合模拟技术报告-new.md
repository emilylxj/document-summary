...
* **$V' = \frac{\partial V}{\partial \rho}$**:几何雅可比因子(体积随半径的变化率)。
...
**实现中的坐标选择**:
输运求解器通常在归一化环向磁通坐标 $\rho \in [0,1]$ 上离散。从极向磁通 $\psi$ 到 $\rho$ 的变换关系为:

$$
\frac{\partial}{\partial \psi} = \frac{d\rho}{d\psi}\frac{\partial}{\partial\rho}, \quad
V'_{\psi}(\psi) = \frac{dV}{d\psi} = \frac{dV}{d\rho}\frac{d\rho}{d\psi} = V'_{\rho}(\rho) \cdot \frac{d\rho}{d\psi}
$$

**关键约定**:
- 本文档第四章的FVM离散推导使用 **$\rho$ 坐标**
- 界面权重因子统一记为 $V'(\rho)$ 或简写为 $V'$
- 当需要与平衡求解器(使用$\psi$)耦合时,通过映射函数 $\rho(\psi)$ 转换

### 3.2 物理含义
* **非线性**:系数 $D, v_{conv}, S$ 通常强依赖于 $u$ 及其梯度 $\nabla u$。例如,湍流热扩散系数 $\chi \propto T^{3/2}$ 或更复杂的湍流模型。
...
- **边缘处 ($\rho=\rho_{sep}$ 或 $\rho_{ped}$)**:这是最棘手的地方。
    ...
    - **耦合逻辑**:TORAX 需要留出接口,允许根据当前的 $\beta_p$ 和成形参数,动态调用 EPED 计算台基高度,从而更新边界条件。
...
$$
\frac{\partial n}{\partial t} = -\frac{1}{V'} \frac{\partial}{\partial \rho} (V' \Gamma) + S
$$

两边同乘 $V'$(即 $dV/d\rho$):

$$
V' \frac{\partial n}{\partial t} = - \frac{\partial}{\partial \rho} (V' \Gamma) + S V'
$$

> 💡 注意:$V' d\rho = dV$,因此左边为 $\frac{\partial n}{\partial t} dV$。

现在,对第 $i$ 个控制体(磁面范围 $\rho \in [\rho_{i-1/2}, \rho_{i+1/2}]$)积分:

$$
\int_{\rho_{i-1/2}}^{\rho_{i+1/2}} V' \frac{\partial n}{\partial t} \, d\rho 
= - \int_{\rho_{i-1/2}}^{\rho_{i+1/2}} \frac{\partial}{\partial \rho} (V' \Gamma) \, d\rho 
+ \int_{\rho_{i-1/2}}^{\rho_{i+1/2}} S V' \, d\rho
$$
...
$$
\int_{\rho_{i-1/2}}^{\rho_{i+1/2}} V' \frac{\partial n}{\partial t} \, d\rho 
= \frac{\partial}{\partial t} \int_{\rho_{i-1/2}}^{\rho_{i+1/2}} n V' \, d\rho 
\approx \frac{\partial n_i}{\partial t} \int_{\rho_{i-1/2}}^{\rho_{i+1/2}} V' \, d\rho 
= \frac{\partial n_i}{\partial t} \Delta V_i
$$

> 📝 **说明**:假设单元内密度均匀($n \approx n_i$),可将 $n_i$ 提出积分号。

**中间项:通量的净流出(用微积分基本定理)**

$$
- \int_{\rho_{i-1/2}}^{\rho_{i+1/2}} \frac{\partial}{\partial \rho} (V' \Gamma) \, d\rho 
= - \left[ (V' \Gamma) \Big|_{\rho_{i+1/2}} - (V' \Gamma) \Big|_{\rho_{i-1/2}} \right] 
= - \left( W_{i+1/2} \Gamma_{i+1/2} - W_{i-1/2} \Gamma_{i-1/2} \right)
$$

其中:

- $W_{i\pm1/2} = V'(\rho_{i\pm1/2})$:**界面几何权重因子**(在磁面平均后的有效"截面"贡献);
- $\Gamma_{i\pm1/2}$:界面处的粒子通量。

> 📝 **说明**:**这是 FVM 的核心——将空间导数转化为"界面通量差"。**  
> ⚠️ **术语澄清**:$W$ 或 $V'$ 在1.5D输运中不是几何意义的二维面积,而是磁面平均后的**雅可比权重**。

### 右边:源项积分

$$
\int_{\rho_{i-1/2}}^{\rho_{i+1/2}} S V' \, d\rho 
\approx S_i \int_{\rho_{i-1/2}}^{\rho_{i+1/2}} V' \, d\rho 
= S_i \Delta V_i
$$
...
$$
\frac{\partial n_i}{\partial t} \Delta V_i = - \left( W_{i+1/2} \Gamma_{i+1/2} - W_{i-1/2} \Gamma_{i-1/2} \right) + S_i \Delta V_i
$$
...
- \left[ \theta \left( W \Gamma^{n+1} \right)_{\text{net}} + (1 - \theta) \left( W \Gamma^{n} \right)_{\text{net}} \right] 
+ \left( \theta S_i^{n+1} + (1 - \theta) S_i^n \right) \Delta V_i
$$

其中净通量:
$$
\left( W \Gamma \right)_{\text{net}} = W_{i+1/2} \Gamma_{i+1/2} - W_{i-1/2} \Gamma_{i-1/2}
$$
...
$$
\frac{n_i^{n+1} - n_i^n}{\Delta t} \Delta V_i = 
- \theta \left( \Gamma_{i+1/2}^{n+1} W_{i+1/2} - \Gamma_{i-1/2}^{n+1} W_{i-1/2} \right) 
- (1 - \theta) \left( \Gamma_{i+1/2}^{n} W_{i+1/2} - \Gamma_{i-1/2}^{n} W_{i-1/2} \right) 
+ \left( \theta S_i^{n+1} + (1 - \theta) S_i^n \right) \Delta V_i
$$
...
## 4.3 通量离散:将 $\Gamma^{n+1}$ 表示为 $n^{n+1}$ 的线性函数

### (a) 扩散项离散(中心差分)
$$
\left( -D \frac{\partial n}{\partial \rho} \right)_{i+1/2}^{n+1} 
\approx -D_{i+1/2}^{(k-1)} \frac{n_{i+1}^{(k)} - n_i^{(k)}}{\Delta \rho_{i+1/2}}
$$
...
**完整迎风逻辑**:
$$
\left( v n \right)_{i+1/2}^{n+1} \approx 
\begin{cases}
v_{i+1/2}^{(k-1)} \cdot n_i^{(k)}, & \text{if } v_{i+1/2} > 0 \text{ (向外流)} \\
v_{i+1/2}^{(k-1)} \cdot n_{i+1}^{(k)}, & \text{if } v_{i+1/2} < 0 \text{ (向内流)}
\end{cases}
$$
> ⚠️ **关键**:必须根据速度符号选择上游节点,否则在反向流动时会出现非物理振荡。
...
$$
\Gamma_{i+1/2}^{n+1} = 
\underbrace{\left( \frac{D_{i+1/2}^{(k-1)}}{\Delta \rho_{i+1/2}} + v_{i+1/2}^{(k-1)} \right)}_{c_1^{(k-1)}} n_i^{(k)} 
+ \underbrace{\left( -\frac{D_{i+1/2}^{(k-1)}}{\Delta \rho_{i+1/2}} \right)}_{c_2^{(k-1)}} n_{i+1}^{(k)}
$$
...
+ \theta \left[ \left(c_1^{(k-1)} n_i^{(k)} + c_2^{(k-1)} n_{i+1}^{(k)} \right) W_{i+1/2} 
- \left(c_1^{\text{left}} n_{i-1}^{(k)} + c_2^{\text{left}} n_i^{(k)} \right) W_{i-1/2} \right]
$$
...
- **$n_{i-1}^{(k)}$ 的系数**(下对角线):
  $$
a_{i,i-1} = - \theta \cdot c_1^{\text{left}} \cdot W_{i-1/2}
  $$

- **$n_i^{(k)}$ 的系数**(主对角线):
  $$
a_{i,i} = \frac{\Delta V_i}{\Delta t} + \theta \left( c_1^{(k-1)} W_{i+1/2} - c_2^{\text{left}} W_{i-1/2} \right)
  $$

- **$n_{i+1}^{(k)}$ 的系数**(上对角线):
  $$
a_{i,i+1} = \theta \cdot c_2^{(k-1)} \cdot W_{i+1/2}
  $$
...
## 4.5 矩阵组装:基于面的贡献分配(Face-based Assembly)
...
for i in range(N-1):  # 遍历界面 i+1/2
    ...
    # 3. 对左侧单元 i 的贡献
    main_diag[i]   += \theta * c1 * W_face
    upper_diag[i]  += \theta * c2 * W_face  # 影响 n_{i+1}
    
    # 4. 对右侧单元 i+1 的贡献
    lower_diag[i]  += -\theta * c1 * W_face  # 影响 n_i
    main_diag[i+1] += -\theta * c2 * W_face
```
...
*   **时间惯性 (Time Inertia)**: $\Delta V_i / \Delta t$
    *   物理意义: 这一项越大($\Delta t$ 越小),$n_i$ 越倾向于保持原来的值,矩阵越稳定(对角占优)。
*   **扩散流出 (Diffusion Loss)**: $\frac{D}{\Delta r} W$
    *   物理意义: 浓度越高,向外扩散越快,这是一种"损失"机制,所以系数为正(移项到左边)。
*   **对流流出 (Convection Loss)**: $v W$ (如果 $v > 0$)
    *   物理意义: 如果速度向外,粒子被带走。

### B. 非对角线 (Off-Diagonals, $a_{i,i-1}$ 和 $a_{i+1}$)
这是邻居单元 $n_{i-1}$ 和 $n_{i+1}$ 的系数,代表"**邻居对本单元的影响(流入/耦合)**"。
*   **扩散流入 (Diffusion Gain)**: $-\frac{D}{\Delta r} W$
    *   物理意义: 邻居浓度越高,扩散进入本单元的粒子越多。因为在等式左边,所以通常为**负值**。
*   **对流流入 (Convection Gain)**: $-v W$ (如果 $v$ 指向本单元)
...
### 4.7.1 算子分裂法 (Operator Splitting)
...
### 4.7.2 线性 Theta 方法 (Linear Theta Method)
...
### 4.8.1 预测-校正法 (Predictor-Corrector)
...
### 4.8.3 全隐式牛顿-拉夫逊法 (Newton-Raphson) —— TORAX 核心
...
本章阐述为什么需要平衡和输运的自洽迭代,如何实现几何与剖面的自洽演化。

**关键词**:**解决粒子/能量输运与磁通扩散(电流演化)的强耦合求解问题**
...
描述极向磁通 $\psi$ 随时间演化的方程:
$$\frac{1}{\eta_\parallel} \frac{\partial \psi}{\partial t} = \frac{1}{\mu_0 R^2} \nabla \cdot \left( \frac{R^2}{\eta_\parallel} \nabla \psi \right) - j_{\text{ni}}$$

> ⚠️ **术语修正**:这里 $\eta_\parallel = 1/\sigma_\parallel$ 为**电阻率**(resistivity),不是电导率。$\eta_\parallel \propto T_e^{-3/2}$(Spitzer电阻率)。

- **耦合点 A(电阻率 $\eta_\parallel$)**:  
...
- **耦合点 B(欧姆加热 $P_{\text{OH}}$)**:  
  $P_{\text{OH}} = \eta_\parallel j^2 \propto T_e^{-3/2} (\nabla^2 \psi)^2$  
  **物理含义**:欧姆加热同时依赖于温度和电流密度(通过 $\psi$ 的二阶导)。  
...
在托卡马克全放电周期中,'热-电-磁'三个物理过程的耦合具有极强的**刚性(Stiffness)**,分开求解会切断关键的物理反馈链条。具体体现在三个方面:  
...
>     
> 3. **几何度规的动态响应(输运-平衡耦合)**:  
>    特别是在高 $\beta_p$ 运行模式下,压强梯度的变化会瞬时引起 Shafranov 位移,改变磁通面的几何因子 $\langle |\nabla \rho|^2 \rangle$。通过全隐式迭代,确保输运过程能感知到磁面形状的**实时变化**,这对于模拟 VDE(垂直位移事件)或 L-H 转换等瞬态过程是至关重要的。  
>     
...
在许多工况下,输运过程(扩散时间尺度 $\tau_{transport} \sim a^2/\chi$)比平衡几何调整(电流扩散时间尺度 $\tau_{current} \sim \mu_0 a^2 \sigma$)更快。因此:

- **一般策略**:可采用多时间尺度方法,输运每步更新,平衡每若干输运步更新一次。
- **刚性阶段必须同步**:
  - **电流爬升(Ramp-up)**:电流快速变化,$\tau_{current}$ 与 $\tau_{transport}$ 可比,必须强耦合。
  - **强加热瞬态**:NBI突然开启导致 $T_e$ 快速上升,$\eta_\parallel$ 剧变,需同步更新电流。
  - **高 $\beta_p$ 运行**:磁面形状对压强梯度敏感,需频繁更新几何。

> ⚠️ **修正说明**:"输运是快过程,平衡变化是慢过程"这一表述是**有条件的**,取决于运行模式和演化阶段。
...
1.  **输运 $\to$ 平衡 (Transport to Equilibrium)**:
    ...
    * 转换为 G-S 源项:$p'(\psi)$ 和 $FF'(	ext{	ext{	ext{p}(	ext{	ext{	ext{	ext{p}}}}}}}}(	ext{	ext{	ext{	ext{	ext{p}}}})}$. 
...
**本版本(v3.1)主要修正**:
- 明确了坐标系统($\rho$ vs $\psi$)及变换关系
- 修正了"界面面积"为"界面几何权重因子"的精确术语
- 补充了对流项迎风格式的完整符号判断逻辑
- 澄清了时间尺度分离的条件性与适用范围
- 修正了电阻率/电导率符号的物理准确性
